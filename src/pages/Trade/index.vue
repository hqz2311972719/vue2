<template>
    <div>
        ...
        <div class="content">
            ...
            <div @click="$store.commit('user/CHANGE_ADDRESS_DEFAULT_BY_ID',item.id)" v-for="item in $store.state.user.addressList" :key="item.id" class="address clearFix">
            ...
            <div class="detail">
                <h5>商品清单</h5>
                <ul v-for="item in $store.state.order.tradeInfo.orderDetailVoList[0].orderDetailList" class="list clearFix">
                    <li>
        ...
        <div class="money clearFix">
            <ul>
                <li>
                    <b><i>{{$store.state.order.tradeInfo.totalNum}}</i>件商品，总商品金额</b>
                    <span>{{$store.state.order.tradeInfo.totalAmount | currency(2,"￥")}}</span>
                </li>
                <li>
                    <b>返现：</b>
                    <span>0.00</span>
        ...
        <div class="trade">
            <div class="price">应付金额:　<span>{{$store.state.order.tradeInfo.totalAmount | currency(2,"￥")}}</span></div>
...
export default {
    name: "Trade",
    computed:mapGetters("user",["addressDefault"]),
    data(){
        return {
            orderComment:""
        }
    ...
    methods:{
        async submitOrder(){
          ...
          console.log(this)
            const result = await this.$store.dispatch("order/postSubmitOrderAsync",{
    ...
    async mounted(){
        ...
        await this.$store.dispatch("order/getTradeListAsync");
    }
}
</script>
